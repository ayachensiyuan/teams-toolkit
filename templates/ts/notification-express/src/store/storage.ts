...
import { PagedData } from "../internal/interface";
...
export class DefaultConversationReferenceStore {
  ...
  private static instance: DefaultConversationReferenceStore;

  private constructor(storage: LocalFileStorage) {
    this.storage = storage;
  }

  public static getInstance(): DefaultConversationReferenceStore {
    if (!DefaultConversationReferenceStore.instance) {
      DefaultConversationReferenceStore.instance = new DefaultConversationReferenceStore(
        new LocalFileStorage(
          path.resolve(process.env.RUNNING_ON_AZURE === "1" ? process.env.TEMP ?? "./" : "./")
        )
      );
    }
    return DefaultConversationReferenceStore.instance;
  }
...
export default DefaultConversationReferenceStore.getInstance();
